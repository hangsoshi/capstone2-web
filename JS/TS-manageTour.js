...
console.log(login);
const api =
  "http://127.0.0.1:8000/api/ts/tour/all/" + login.user_info.user_profile[0].id;
var i = 0;
let htmls = "";
function getTours(api) {
  fetch(api)
    ...
    .then((data) => {
        ...
        <tr class="list-residence">
                        ...
                        <td class="list-content list-action">
                            <div class="" style="display: flex; align-items: center; justify-content: center; text-align: center;">
                                <button onclick="handleUpdate(${tour.id})" style="border: none; background-color: white">
      ...
      sliderFind.innerHTML =
        htmls.join("") +
        ` <div class="modal-container" id="modal-opened" data-id="${1}">
               <div class="modal">
...
function handleSuccessDelete() {
  fetch(
    "http://127.0.0.1:8000/api/ts/tour/delete/" +
      idDelete +
      "?ts_id=" +
      `${login.user_info.user_profile[0].id}`,
    {
      method: "DELETE",
    }
  )
  ...
  setTimeout(() => {
    fetch("http://127.0.0.1:8000/api/ts/tour/search?name=" + e.target.value)
      ...
      .then((data) => {
        ...
        getSearchTours(
          "http://127.0.0.1:8000/api/ts/tour/search?name=" + e.target.value
        );
      })
      .catch((error) => {
        console.log(error);
